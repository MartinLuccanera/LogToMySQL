...
App was created using Java 8, Spring boot and Maven.

Database schema is generated by liquibase. If DB does not exist, liquibase creates it from schema defined in liquibase-changelog.xml.

Database configuration can be done via applications.properties.

To compile app to .jar 

```
mvn package
``` 

Application can be run with defined parameters 

```
java -jar target/parser-0.0.1-SNAPSHOT.jar --accesslog=access.log  --startDate=2017-01-01.13:00:00 --duration=hourly --threshold=100
```

After the initial run, accesslog parameter can avoided.
...
SELECT l.ip 
FROM log l 
WHERE l.date BETWEEN '2017-01-01.13:00:00' AND '2017-01-01.14:00:00'
GROUP BY l.ip 
...
(2) Source Code for the Java program
    ...
    Available in git at https://github.com/MartinLuccanera/LogToMySQL
